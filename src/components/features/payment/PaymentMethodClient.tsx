...
export function PaymentMethodClient() {
  ...
  const loadCards = async () => {
    try {
      ...
      setError(err.response?.data?.message || '카드 목록을 불러오는데 실패했습니다.')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleAddCard = () => {
    alert('카드 등록은 토스 결제창을 통해서만 가능합니다.\n낙찰 후 결제 시 자동으로 등록됩니다.')
  }
    ...
    <div className="mx-auto max-w-4xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      {isLoading ? (
        <Card variant="outlined">
          <CardContent className="py-16 text-center">
            ...
            <h3 className="text-lg font-semibold text-neutral-900">카드 목록을 불러오는 중...</h3>
          </CardContent>
        </Card>
      ) : error ? (
        <Card variant="outlined">
          <CardContent className="py-16 text-center">
            ...
            <Button onClick={loadCards} variant="outline" className="mt-4">다시 시도</Button>
          </CardContent>
        </Card>
      ) : cards.length === 0 ? (
        <Card variant="outlined">
          <CardContent className="py-16 text-center">
            ...
            <h3 className="mb-2 text-lg font-semibold text-neutral-900">등록된 카드가 없습니다</h3>
            <p className="mb-4 text-neutral-600">경매 낙찰 후 결제 시 토스 결제창을 통해 카드를 등록할 수 있습니다.</p>
          </CardContent>
        </Card>
      ) : (
            ...
            <Card key={card.id} variant="outlined">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h3 className="text-lg font-semibold text-neutral-900">{card.cardName}</h3>
                    <p className="text-sm text-neutral-600">{getCardCompanyName(card.cardCompany)}</p>
                  </div>
                </div>
              ...
              <CardContent>
                <p className="text-sm text-neutral-600">카드번호: {card.cardNumberMasked}</p>
              </CardContent>
            </Card>
          ))}
        </div>
      ...
      <Card variant="outlined" className="mt-6 border-blue-200 bg-blue-50">
        <CardContent className="p-4">
          <h4 className="mb-2 text-sm font-semibold text-blue-900">💡 카드 등록 안내</h4>
